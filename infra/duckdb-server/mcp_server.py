...
def sql_query(sql: str, ctx: Context) -> ToolResult:
    ...
    All tables live in the 'lake' database.
...
def sql_admin(sql: str, ctx: Context) -> str:
    """Execute DDL statements (CREATE OR REPLACE VIEW only).
    Use this to create views that parse JSON columns, add computed columns, etc.
    Example: CREATE OR REPLACE VIEW lake.housing.v_designated_buildings AS
             SELECT *, json_extract_string(the_geom, '$.coordinates[0]')::DOUBLE AS lon,
                       json_extract_string(the_geom, '$.coordinates[1]')::DOUBLE AS lat
...
def list_schemas(ctx: Context) -> str:
    """List all schemas in the NYC data lake with table counts and total rows."""
...
def list_tables(schema: str, ctx: Context) -> ToolResult:
    """List all tables in a schema with row counts and column counts."""
    catalog = ctx.lifespan_context["catalog"]
    db = ctx.lifespan_context["db"]
...
def describe_table(schema: str, table: str, ctx: Context) -> ToolResult:
    """Describe a table's columns: names, types, and whether nullable."""
    db = ctx.lifespan_context["db"]
    catalog = ctx.lifespan_context["catalog"]
    schema = _fuzzy_match_schema(db, schema, catalog)
...
def data_catalog(ctx: Context, keyword: str = "") -> ToolResult:
    ...
    Uses fuzzy matching — typos and partial terms work fine.
...
def text_search(query: str, ctx: Context, corpus: str = "all", limit: int = 20) -> ToolResult:
    """Search NYC open data by keyword (ILIKE on lake tables — no in-memory copies).
    Corpora: 'financial' (CFPB complaints), 'restaurants' (inspection violations), 'all' (both).
    Examples: 'identity theft credit report', 'mice droppings kitchen',
    'Wells Fargo unauthorized', 'cold food temperature violation'"""
    if not query.strip():
...
def restaurant_lookup(name: str, ctx: Context, zipcode: str = "", borough: str = "") -> ToolResult:
    """Look up a NYC restaurant by name — fuzzy matching, typos OK.
    Returns current grade, inspection history, and recent violations.
    Optionally filter by ZIP code or borough (Manhattan, Brooklyn, etc.).
    Examples: restaurant_lookup("Wo Hop"), restaurant_lookup("joes pizza", borough="Manhattan")"""
    if len(name.strip()) < 2:
...
def area_snapshot(lat: float, lng: float, ctx: Context, radius_m: int = 500) -> ToolResult:
    """Get a comprehensive snapshot of a NYC location — what's nearby?
    Uses spatial queries across crimes, restaurants, subway, 311, rats, and trees.
    Provide latitude and longitude coordinates. Default radius: 500 meters.
    Examples: area_snapshot(40.7128, -74.0060) for Lower Manhattan,
    area_snapshot(40.6892, -73.9857, radius_m=300) for tight radius"""
...
def neighborhood_compare(zip_codes: list[str], ctx: Context) -> ToolResult:
    """Compare NYC neighborhoods across quality-of-life dimensions.
    Provide 2-7 ZIP codes to compare: crime, restaurants, housing quality,
    noise, income, commute time, parks, and education levels.
    Great for deciding where to live, work, or invest.
    Example: neighborhood_compare(["10003", "11201", "11215"])"""
...
def gentrification_tracker(zip_codes: list[str], ctx: Context) -> ToolResult:
    """Track gentrification & displacement pressure for NYC neighborhoods.
    Analyzes 5 quarterly signals since 2020 + forecasts 4 quarters ahead:
    - New restaurant openings (economic investment)
    - HPD housing complaints (tenant turnover signal)
    - 311 noise/NYPD calls (nightlife & activity growth)
    - DOB ECB violations (construction/renovation boom)
    - DOB 311 complaints (construction activity from residents)
    Plus: IRS income trend, HMDA investor/lending activity, eviction trends,
    recent rezonings, affordable housing, and racial lending disparities.
    Provide 1-5 ZIP codes. Example: gentrification_tracker(["11237", "11221"])"""
...
def safety_report(precinct: int, ctx: Context) -> ToolResult:
    """Comprehensive safety & policing report for an NYPD precinct.
    Covers crime trends, arrest demographics, enforcement intensity,
    shootings, summons, hate crimes, CCRB misconduct, Use of Force,
    and officer discipline.
    Precinct numbers: 1-123. Example: safety_report(75) for East New York,
    safety_report(14) for Midtown South, safety_report(1) for Financial District"""
...
def landlord_watchdog(bbl: str, ctx: Context) -> ToolResult:
    """Investigate a landlord's FULL portfolio — all buildings they own, violations,
    complaints, evictions, enforcement actions. Exposes slumlord patterns across
    their entire portfolio, not just one building.

    Input: BBL (10 digits: borough(1) + block(5) + lot(4)). Example: 1000670001

    Returns: building info, portfolio size, aggregated violations/complaints,
    enforcement flags (AEP, CONH, Underlying Conditions), slumlord score,
    and actionable next steps (hotlines, tenant rights, legal aid).
    """
    if not re.match(r"^\d{10}$", bbl):
        raise ToolError(
            "BBL must be exactly 10 digits. Format: borough(1) + block(5) + lot(4)"
...
def environmental_justice(zipcode: str, ctx: Context) -> ToolResult:
    """Environmental justice report for a NYC ZIP code. Exposes pollution burden,
    health outcomes, environmental racism, and waste inequity. Compares to city
    averages. Covers: air quality (PM2.5, NO2), EJ area status, waste facilities,
    EPA sites, rats, trees, 311 environmental complaints, asthma/mortality,
    flood/heat vulnerability, and DSNY waste tonnage.

    Input: 5-digit NYC ZIP code. Example: 10456 (South Bronx), 11233 (Bed-Stuy),
    10035 (East Harlem). Returns environmental burden score and actionable steps.
    """
...
def resource_finder(zipcode: str, need: str, ctx: Context) -> ToolResult:
    """Find social services and resources near a NYC ZIP code.
    Covers: food (pantries, SNAP, farmers markets), health (clinics, mental health),
    childcare, youth programs (DYCD), benefits (SNAP, Medicaid), legal aid,
    domestic violence (Family Justice Centers), Wi-Fi, senior services, and more.

    Input:
      zipcode: 5-digit NYC ZIP. Example: 10456, 11233, 10035
      need: what you're looking for. Examples: "food", "health", "childcare",
            "youth", "benefits", "legal", "shelter", "wifi", "senior", "all"
    """
    if not re.match(r"^\d{5}$", zipcode):
        raise ToolError("ZIP code must be exactly 5 digits")
...
def school_report(dbn: str, ctx: Context) -> ToolResult:
    """Comprehensive report for a NYC public school by DBN (District-Borough-Number).
    DBN format: 2-digit district + borough letter + 3-digit school number.
    Examples: 02M001 (PS 1, Manhattan), 13K330 (Brooklyn), 10X291 (Bronx).
    Borough letters: M=Manhattan, X=Bronx, K=Brooklyn, Q=Queens, R=Staten Island.
    Returns: directory info, performance scores, test results (ELA + Math), and safety."""
    dbn = dbn.strip().upper()
...
def commercial_vitality(zipcode: str, ctx: Context) -> ToolResult:
    """Measure commercial health and business activity for a NYC ZIP code.
    Analyzes business licenses, restaurants, DOB permits, sidewalk cafes,
    certified businesses, and commercial 311 complaints.
    Input: 5-digit NYC ZIP. Example: 10003 (East Village), 11201 (Downtown Brooklyn)."""
    if not re.match(r"^\d{5}$", zipcode):
        raise ToolError("ZIP code must be exactly 5 digits")
...
def building_story(bbl: str, ctx: Context) -> ToolResult:
    """The life story of a NYC building — its era, what it's survived, how it
    compares to its neighbors, and what the data reveals about its character.

    Input: BBL (10 digits: borough(1) + block(5) + lot(4)). Example: 1000670001

    Returns: historical context, building facts, complaint character,
    neighborhood comparison, and notable milestones the building has witnessed."""
...
def neighborhood_portrait(zipcode: str, ctx: Context) -> ToolResult:
    """What makes a NYC neighborhood distinctive — its cuisine fingerprint,
    building stock, noise level, business mix, and how it compares to the city.

    Input: ZIP code (5 digits). Example: 10003

    Returns: cuisine profile, restaurant grades, building stats, 311 character,
    business mix, and what makes this ZIP statistically unique."""
...
def nyc_twins(bbl: str, ctx: Context) -> ToolResult:
    """Find buildings across NYC that are statistically similar to yours —
    same era, same class, similar size — and compare how they're doing.

    Input: BBL (10 digits: borough(1) + block(5) + lot(4)). Example: 1000670001

    Returns: your building's key facts, up to 5 twin buildings in other boroughs,
    comparison on violations and assessed value, and total count of similar buildings citywide."""
...
def block_timeline(bbl: str, ctx: Context) -> ToolResult:
    """Your block through time — what's been built, opened, complained about,
    and how the block has changed over the last decade.

    Input: BBL (10 digits). Uses the block portion to find all buildings on your block.

    Returns: all buildings on the block, construction permits by year,
    restaurant scene, 311 complaint trends, and HPD complaint patterns."""
...
def building_context(bbl: str, ctx: Context) -> ToolResult:
    """What was happening when your building was born — famous contemporaries,
    citywide construction activity, and historical context.

    Input: BBL (10 digits: borough(1) + block(5) + lot(4)). Example: 1000670001

    Returns: year built, era context, famous buildings from the same era,
    how many buildings were built the same year citywide, borough breakdown,
    and what your neighborhood looked like at the time."""
...
def landlord_network(bbl: str, ctx: Context) -> ToolResult:
    """Discover the full ownership network around a building using graph traversal.
    Starting from a BBL, finds the owner, ALL their other buildings, and aggregated
    violation stats across the entire portfolio. Uses DuckPGQ property graph."""
    _require_graph(ctx)
    if not re.match(r"^\d{10}$", bbl):
        raise ToolError("BBL must be exactly 10 digits")
...
def ownership_graph(bbl: str, depth: int = 2, ctx: Context = None) -> ToolResult:
    """Trace ownership connections between buildings via shared landlords.
    Starting from a BBL, follows building-to-building SharedOwner links.
    Depth 1 = buildings sharing a direct owner. Depth 2+ = indirect connections
    (buildings sharing owners with your owner's other buildings).
    Uses DuckPGQ shortest path on the building network graph."""
    _require_graph(ctx)
    if not re.match(r"^\d{10}$", bbl):
        raise ToolError("BBL must be exactly 10 digits")
...
def ownership_clusters(ctx: Context, min_buildings: int = 5, borough: str = "") -> ToolResult:
    """Find clusters of buildings connected through shared ownership using
    Weakly Connected Components (WCC). Reveals hidden ownership networks —
    groups of buildings linked through shared landlords, even across different
    corporate names. Optionally filter by borough (Manhattan/Bronx/Brooklyn/Queens/Staten Island).
    min_buildings filters to clusters with at least N buildings (default 5)."""
    _require_graph(ctx)
    db = ctx.lifespan_context["db"]
    t0 = time.time()
...
def ownership_cliques(ctx: Context, top_n: int = 20) -> ToolResult:
    """Find buildings at the center of tight-knit ownership networks using
    Local Clustering Coefficient (LCC). High LCC means a building's co-owned
    neighbors are also co-owned with each other — indicating concentrated
    ownership cliques (potential shell company networks or portfolio landlords)."""
    _require_graph(ctx)
    db = ctx.lifespan_context["db"]
    t0 = time.time()
...
def worst_landlords(ctx: Context, borough: str = "", top_n: int = 25) -> ToolResult:
    """Find the worst landlords in NYC using a composite score across violations,
    litigations, harassment findings, AEP buildings, tax liens, evictions, and complaints.
    Uses DuckPGQ ownership graph to aggregate across entire portfolios.
    Score = ClassC*3 + Litigations*5 + Harassment*10 + AEP*8 + OpenViol + Liens*2.
    Optionally filter by borough name or code (1-5)."""
    _require_graph(ctx)
    db = ctx.lifespan_context["db"]
    t0 = time.time()
...
def llc_piercer(entity_name: str, ctx: Context) -> ToolResult:
    """Pierce the LLC veil — given a corporate entity name, find the people
    and addresses behind it using NYS Department of State corporation filings,
    ACRIS property transaction records, and HPD registration contacts.
    Searches fuzzy-match across all three data sources."""
...
def entity_xray(name: str, ctx: Context) -> ToolResult:
    """Full X-ray of a person or entity across ALL NYC datasets. Searches 23 sources:
    NYS corporations, business licenses, restaurant inspections, campaign contributions,
    campaign expenditures, OATH hearings, ACRIS transactions, PLUTO property portfolio,
    DOB permits, DCWP consumer charges, SBS M/WBE certifications, citywide payroll,
    DOB application owners, Doing Business disclosures, EPA ECHO facilities,
    NYC marriage index (1950-2017), NYS attorney registrations, ACRIS personal property
    (UCC filings), civil service active lists, NYS lobbyist registrations,
    NYS death index, NYS real estate brokers, and NYS notaries.
    Handles name variations — 'Barton Perlbinder' matches 'Perlbinder, Barton M'.
    Reveals the full empire — buildings, businesses, restaurants, political donations,
    city contracts, property deals, permits, environmental violations, family ties, and more."""
    if len(name.strip()) < 3:
...
def marriage_search(surname: str, first_name: str = "", ctx: Context = None) -> ToolResult:
    """Search NYC marriage records (1866-2017, ~10M records) by surname.
    Combines three sources: marriage certificates 1866-1937 (5.3M records),
    marriage index 1950-2017 (4.7M parquet), covering 150+ years.
    Matches both bride and groom surnames. Optionally filter by first name.
    Use this to confirm family relationships — co-signers on a marriage license
    prove familial connection. Data from Reclaim The Records FOIL lawsuits."""
...
def property_history(bbl: str, ctx: Context) -> ToolResult:
    """Full transaction history of a NYC property — every sale, mortgage,
    satisfaction, and assignment recorded in ACRIS since 1966.
    Shows the chain of ownership: who bought from whom, when, for how much,
    plus mortgage activity. 85M+ records across the ACRIS triplet.
...
def enforcement_web(bbl: str, ctx: Context) -> ToolResult:
    """Full multi-agency enforcement picture for a NYC property — HPD violations,
    DOB ECB violations, FDNY violations, OATH hearings, restaurant inspections,
    DOB complaints, facade safety (FISP), boiler safety, and SRO status.
    Shows every agency that has taken action at this address, with severity scoring.
    Uses the 21.5M-row OATH hearings table as the enforcement hub.
...
def flipper_detector(ctx: Context, borough: str = "", months: int = 24, min_profit_pct: int = 20) -> ToolResult:
    """Detect property flippers — investors who buy and resell NYC properties
    within a short window for profit. Finds properties with 2+ deed transactions
    within N months where price increased by at least min_profit_pct%.
    Cross-references with DOB permits (renovation between buy/sell) and
    tax lien sales (distressed acquisition). Covers 16.9M ACRIS transactions.

    Optionally filter by borough name or code (1-5). Default: all boroughs.
    months: max months between buy and sell (default 24).
    min_profit_pct: minimum price increase percentage (default 20)."""
    db = ctx.lifespan_context["db"]
    t0 = time.time()
...
def transaction_network(name: str, ctx: Context) -> ToolResult:
    """Trace a person or entity through NYC property transactions using DuckPGQ
    graph traversal. Starting from an entity name, finds all properties they
    bought/sold, all co-transactors (other parties on the same deeds), and
    traverses up to 3 hops through the transaction network.
    Reveals hidden connections between buyers, sellers, and shell companies.

    Input: entity name (person or company). Example: 'BLACKSTONE'"""
    _require_graph(ctx)
    if len(name.strip()) < 3:
        raise ToolError("Name must be at least 3 characters")
...
def corporate_web(entity_name: str, ctx: Context) -> ToolResult:
    """Expose the corporate web behind a NYC property entity using DuckPGQ.
    Starting from a company or person name, traverses NYS corporation filings
    to find all connected LLCs, shared officers, shared addresses, and links
    to HPD landlord registrations. Reveals shell company networks.

    Input: entity or person name. Example: 'KUSHNER'"""
    _require_graph(ctx)
    if len(entity_name.strip()) < 3:
        raise ToolError("Name must be at least 3 characters")
...
def pay_to_play(entity_name: str, ctx: Context) -> ToolResult:
    """Trace political influence networks using DuckPGQ. Starting from any
    political entity (donor, candidate, lobbyist, vendor), follows the money
    through donations, lobbying, and city contracts. Exposes pay-to-play:
    who donates → who gets contracts → who lobbies for whom.

    Input: entity name. Example: 'CATSIMATIDIS'"""
    _require_graph(ctx)
    if len(entity_name.strip()) < 3:
        raise ToolError("Name must be at least 3 characters")
...
def contractor_network(name_or_license: str, ctx: Context) -> ToolResult:
    """Expose a contractor's full NYC footprint using DuckPGQ. Given a contractor
    name or license number, shows all buildings they've worked on, violation rates
    at those buildings, other contractors they co-work with, and the owners they
    serve. Reveals contractor rings and safety patterns across 4M+ DOB permits.

    Input: contractor name or license number. Example: 'NACHMAN PLUMBING' or '0042962'"""
    _require_graph(ctx)
...
def shell_detector(ctx: Context, borough: str = "", min_corps: int = 5) -> ToolResult:
    """Find clusters of shell companies connected through shared officers using
    DuckPGQ Weakly Connected Components. Reveals hidden corporate networks —
    groups of LLCs linked through the same people, even across different names.
    These clusters often indicate a single beneficial owner hiding behind
    multiple corporate entities. Optionally filter by borough/county."""
    _require_graph(ctx)
    db = ctx.lifespan_context["db"]
    t0 = time.time()
