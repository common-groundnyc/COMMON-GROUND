...
def make_app() -> Starlette:
    ...
    async def neighborhood(request):
        return await neighborhood_endpoint(request, pool=fake_pool)

    async def zips_search(request):
        return await zips_search_endpoint(request, pool=fake_pool)

    async def worst_buildings(request):
        return await worst_buildings_endpoint(request, pool=fake_pool)

    return Starlette(routes=[
        Route("/api/neighborhood/{zip_code}", neighborhood),
        Route("/api/zips/search", zips_search),
        Route("/api/buildings/worst", worst_buildings),
    ])
...
def test_neighborhood_returns_stat_card_payload(mock_execute):
    mock_execute.return_value = (
        ["violations_count", "complaints_count", "crimes_count", "restaurants_a_pct"],
        [(1847, 4210, 892, 78.0)],
    )
...
def test_neighborhood_clamps_days_to_3650():
    ...
    with patch("routes.explore.execute") as mock_execute:
        mock_execute.return_value = (["a", "b", "c", "d"], [(0, 0, 0, 0.0)])
...
def test_worst_buildings_returns_ranked_list(mock_execute):
    ...
    mock_execute.return_value = (
        ["bbl", "address", "violation_count", "class_c_count", "last_violation_date"],
        [
